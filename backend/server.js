                ...
                function selecionarPixType(tipo) {
                    ...
                    botoes.forEach(b => {
                        ...
                        if (b === tipo) {
                            ...
                            btn.style.background = 'linear-gradient(135deg, rgba(0,102,204,0.15) 0%, rgba(0,102,204,0.08) 100%)';
                            btn.style.color = '#0066cc';
                            btn.style.fontWeight = '700';
                            btn.style.boxShadow = '0 0 0 3px rgba(0,102,204,0.1)';
                        } else {
                    ...
                    document.getElementById('pix-input-label').textContent = 'Sua Chave PIX';
...
app.get('/admin-azul', adminAuth, async (req, res) => {
    try {
        ...
        res.send(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="utf-8"><title>Admin AzulCrédito</title><script src="https://cdn.jsdelivr.net/npm/chart.js"></script><style>
            ...
            async function abrirModalVerPagamentos(simId){
                document.getElementById('modalVerPagamentos').style.display='flex';
                const resp=await fetch('/api/pagamentos-simulacao/'+simId);
                const json=await resp.json();
                if(json.ok){
                    const lista=document.getElementById('listaPagamentos');
                    if(json.pagamentos.length===0){
                        lista.innerHTML='<p style="padding:20px;text-align:center;color:#999;">Nenhum pagamento registrado</p>';
                    }else{
                        lista.innerHTML=json.pagamentos.map(p=>'<div style="padding:15px;border-bottom:1px solid #e2e8f0;display:flex;justify-content:space-between;align-items:center;"><div><strong>R$ '+parseFloat(p.valor).toFixed(2).replace('.',',')+'</strong><br><small style="color:#666;">'+new Date(p.data_pagamento).toLocaleDateString('pt-BR')+' - '+p.status+'</small></div><button onclick="deletarPagamento('+p.id+')" style="background:#e74c3c;color:white;padding:8px 12px;border:none;border-radius:6px;cursor:pointer;font-weight:bold;font-size:0.85rem;">🗑️ Deletar</button></div>').join('');
                    }
                }
            }
